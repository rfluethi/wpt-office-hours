...
├── notes/
│   ├── README.md                    ← How notes are filed and published
│   ├── YYYY-MM-DD-agenda.md         ← Agenda (published before each meeting)
│   └── YYYY-MM-DD-meeting.md        ← Notes draft (published after each meeting)
└── templates/
    ├── agenda.md                    ← Reusable agenda template for moderators
    └── meeting-notes.md             ← Reusable notes template for note-takers
```
...
1. Note-taker creates a draft in `/notes/` during or after the meeting
2. Participants have ~48 hours to comment and suggest corrections
3. Once approved, notes are published as a blog post on [make.wordpress.org/training](https://make.wordpress.org/training/)
4. The file is updated with a link to the published post
